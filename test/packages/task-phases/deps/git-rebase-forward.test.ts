 ...
 *
 * **Correction (architect fix, build-phase review):** §3.3's expected
 * `subjects()` list originally ended in a single `"init"`. The fixture
 * itself creates two commits titled `"init"` on `main` — `initRepo()`'s own
 * internal empty commit, then a separate explicit `createCommit(repo,
 * "init.txt", "init", "init")` right after — so `main`'s real history
 * genuinely carries `"init"` twice at its root. Only surfaced once a real
 * `rebase()`/`mergeBase()` implementation existed to run the fixture
 * against and reach this assertion at all — every scenario failed
 * uniformly at the earlier outcome check before that, which is exactly why
 * this couldn't have been caught during architect review of the
 * pre-implementation test file. Corrected to list `"init"` twice,
 * matching the fixture's own actual setup.
 */
