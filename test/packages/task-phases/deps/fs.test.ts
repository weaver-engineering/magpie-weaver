 ...
 * readDir, §3.3 loadConfig, §3.4 error handling.
 *
...
/** Make `dir` a git repository (no commit) so the "repo root" for
 *  `loadConfig`'s walk-up is well-defined. */
...
describe("§3.3 loadConfig", () => {
  ...
  it("finds .task-phases.json in the repo root from a nested cwd (§3.3.1)", () => {
    const dir = tempDir();
    try {
      // Given: a repo whose root has .task-phases.json with a custom tasks.docs
      initRepoRoot(dir);
      writeFixtureFile(
        dir,
        ".task-phases.json",
      ...
      // Given: the command is invoked from a subdirectory several levels deep
      const nested = join(dir, "a", "b", "c", "d");
      mkdirSync(nested, { recursive: true });

      // When — loadConfig (no args)
      const { stdout, status } = runCli(nested, ["--dev-testing", "fs", "loadConfig"]);

      // Then — exits 0 and the reported tasks.docs matches the repo-root file
      expect(status).toBe(0);
  ...
  it("reports a missing config rather than crashing (§3.3.2)", () => {
    const dir = tempDir();
    try {
      // Given: a repo with no .task-phases.json anywhere from cwd to root
      initRepoRoot(dir);
