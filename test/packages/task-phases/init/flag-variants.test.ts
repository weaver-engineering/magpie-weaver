...
describe("init: --doc / --specs / --wip flag variants", () => {
  ...
  it("a bad --specs path warns and continues — the remaining specs still copy, exit 0 (§3.4)", async () => {
    const { tools, mocks } = buildTools();
    // The --specs loop copies unconditionally and treats a rejected
    // copyFile as "this path can't be copied" (mirroring how a real
    // ENOENT would surface) — it never pre-checks fileSystem.exists() the
    // way --doc does. The failure signal is the second call rejecting,
    // not the destination path itself; order matches the loop's own
    // sequential iteration over the given --specs paths.
    mocks.copyFile
      .mockResolvedValueOnce(undefined)
      .mockRejectedValueOnce(
        new Error("ENOENT: no such file or directory, open 'missing-spec.md'"),
      );
    const cap = captureStdout();
    const code = await run(
      [
        "node", "cli.js", "init", "AAA-009", "--title", "x",
        "--specs", "good-spec.md", "missing-spec.md", "--json",
      ],
      tools,
    );
    const stdout = cap.stdout();
    ...
    // The failing copy WAS attempted (that's how the failure is detected —
    // there's no other signal in this design) but its destination never
    // lands in the reported specDocPaths, since the loop only records a
    // path after copyFile actually resolves.
    expect(mocks.copyFile).toHaveBeenCalledWith(
      "missing-spec.md",
      "docs/tasks/AAA-009/task-AAA-009-02-spec.md",
    );
    const lines = stdout.trim().split("\n");
    const doc = JSON.parse(lines[lines.length - 1]) as {
      result: { specDocPaths: string[] };
    };
    expect(doc.result.specDocPaths).toEqual(["docs/tasks/AAA-009/task-AAA-009-01-spec.md"]);
  });
