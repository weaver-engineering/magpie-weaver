 ...
 * spec.md, LLD §3.2).
 *
 * **Correction (architect fix, build-phase review, ahead of merge):** the
 * regular route's Main Gate PR pair is `ready/{ref}` -> `main`, not
 * `build/{ref}` -> `main` — confirmed against every real Main Gate PR in
 * this project's history (`ready/{ref}` is what build-implementer actually
 * raises, per the `main/{ref}` -> `ready/{ref}` rename recorded in
 * `task-MAG-40.md`; `build/{ref}` is a static staging branch `ready/{ref}`
 * forks from, never the PR head itself). The original mocks/assertions
 * here checked `findMergedPR("main", "build/{ref}")`, matching a stale
 * assumption also baked into `repo-state.ts`'s `GATE_PR_PAIRS` and the LLD
 * — caught via a real e2e fixture (a genuinely merged Main Gate PR using
 * the real branch pattern), which the mocked suite's self-consistent but
 * factually wrong fixture couldn't catch. Fixed to `findMergedPR("main",
 * "ready/{ref}")`; `canonicalBranch`/`deleteBranch` targets are unaffected
 * — `build/{ref}` remains correct there, this only changes which branch
 * the merged-PR *lookup* checks.
 *
 * Two behaviors under test, kept separate per §3.1-§3.2:
 *   1. Merged Main Gate PR (`ready/{ref}` -> `main`) + local `main`'s HEAD
...
function buildTools(
  ...
  const findMergedPR =
    overrides.findMergedPR ?? mergedPRFor(["main", `ready/${ref}`], mergedMainGatePR());
...
describe("status: merged Main Gate PR with local main not yet caught up (§3.1)", () => {
  it("derives phase build / state merged-pending-cleanup, confirming main hasn't caught up via isAncestor", async () => {
    ...
    expect(mocks.findMergedPR).toHaveBeenCalledWith("main", "ready/AAA-123");
...
describe("status: interrupted prior cleanup — surviving branch already an ancestor of main (§3.2)", () => {
  it("derives the SAME build / merged-pending-cleanup, not a distinct stale or broken state", async () => {
    ...
    expect(mocks.findMergedPR).toHaveBeenCalledWith("main", "ready/AAA-124");
