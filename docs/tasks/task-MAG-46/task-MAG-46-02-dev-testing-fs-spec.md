...
* Given
  * `.task-phases.json` exists several directories **above** the repo
    root (outside the repo entirely) with a custom `tasks.docs` value
  * No `.task-phases.json` exists anywhere inside the repo itself
...
* Then - the reported config's `tasks.docs` matches the outside-the-repo
  file's value, not any default. The walk-up must not stop at the repo
  root — a config file can legitimately live above it.
