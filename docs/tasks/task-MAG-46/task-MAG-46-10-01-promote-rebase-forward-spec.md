...
- **§3.1 only: `promote` must restore the caller's starting branch after
  a successful (or conflicted) rebase.** `rebase()`'s own contract (LLD
  §4.8) is `git rebase --onto <ontoRef> <upstream> <branch>` — git's
  three-argument form, which checks out `<branch>` as part of the
  operation and leaves it checked out afterward. In §3.1's scenario the
  caller is on `spec/{ref}`, rebasing the *different* branch
  `test/{ref}` — so after `rebase("test/{ref}", "spec/{ref}")` runs, the
  worktree is left parked on `test/{ref}`, reproducing MAG-46-10's own
  fork bug (worktree-exclusivity: whichever worktree is doing the test
  phase can no longer check `test/{ref}` out). Same fix, same LLD §2.1
  invariant: `promote` must call `checkout("spec/{ref}")` afterward to
  restore the branch it was called from, whenever `rebase()`'s own
  `git rebase --onto` actually ran — §3.1 (`ok`) and §3.5 (`conflict`,
  discovered mid-replay, after the checkout already happened). **Not**
  §3.6: `rebase()`'s commit-count precondition (LLD §4.8) is a plain
  `rev-list --count`, checked *before* any checkout is attempted, so
  `unexpected-commit-count` means the worktree never moved and there's
  nothing to restore. **§3.2/§3.3 need no restoration either** — there
  the branch being rebased (`spec/{ref}`/`task/{ref}`) already *is* the
  currently-checked-out branch, so `rebase()`'s internal checkout is a
  no-op and the caller ends up exactly where it should.
...
* Then -
  ...
  * `git.checkout("spec/AAA-123")` was called after that — restoring the
    caller's starting branch (§2.1's correction; `rebase`'s own
    `--onto`/`<branch>` form leaves `test/AAA-123` checked out)
  * `git.currentBranch()` ends equal to `"spec/AAA-123"`, where it started
...
* Then -
  ...
  * `git.checkout("spec/AAA-123")` was still called — the caller's
    starting branch is restored regardless of outcome, since `rebase()`
    already moved the worktree before reporting the conflict
...
* Then -
  * `git.push(...)` was **not** called
  * `git.checkout(...)` was **not** called — `rebase()`'s commit-count
    precondition (LLD §4.8) is checked via `rev-list --count`, purely by
    ref name, *before* any `git rebase --onto`/checkout is attempted;
    `unexpected-commit-count` means that precondition failed, so the
    worktree never moved and there's nothing to restore (contrast §3.5,
    where the checkout already happened before the conflict was found)
